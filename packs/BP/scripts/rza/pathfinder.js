...
class PathFinder {
    ...
    blockCache = new Map(); // Cache block check results
    lastFailedPos = null; // Track last failed position
    spreadLevel = 1; // Track current spread level
    POINT_SPACING = 1; // Distance between points in blocks
    CORNER_SMOOTHNESS = 5; // Higher = smoother corners (1-5 recommended)
    /**
    ...
    async findPath(start) {
        ...
        while (this.openSet.length > 0 && iterations < maxIterations) {
            ...
            if (iterations > 300 && iterations % 50 === 0)
                await system.waitTicks(1); // Throttle loop
